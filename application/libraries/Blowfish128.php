...
class Blowfish128 {
  ...
  function Blowfish128($key, $mode, $random=array(), $padding, $iv=NULL) {
    $this->start = 0;
    $this->end = 0;
    $this->mode = $mode;
	...
	$this->rand = $random;
   //odd
    //$this->rand = array(1, 2, 3, 5, 6, 9, 12, 15);
    sort($this->rand);
  }
   ...
   * @author Matt Harris
   **/
  public static function encrypt($plaintext, $key, $random=array(), $mode=Blowfish128::BLOWFISH_MODE_CBC, $padding=Blowfish128::BLOWFISH_PADDING_RFC, $iv=NULL) {
    if ( $mode == Blowfish128::BLOWFISH_MODE_CBC and empty($iv) ) {
      throw new Exception('CBC Mode requires an IV key');
      return;
    }
    $ciphertext = '';
    $fish = new Blowfish128($key, $mode, $random, $padding, $iv);
    ...
    for ($i=0; $i < $len; $i+=$block) {
      if ($mode == Blowfish128::BLOWFISH_MODE_CBC) {
        $chain = ($i == 0) ? $fish->IV : substr($ciphertext, $i - $block, $block);
        list(, $_xL, $_xR) = unpack('N2', substr($paded, $i, $block) ^ $chain);
      } else {
        list(, $_xL, $_xR) = unpack('N2', substr($paded, $i, $block));
      }

      if($cnt%2 == 0){
      // Store the 32bit xL and xR
        $code[$code_cnt]['xL'] =  $_xL;
        $code[$code_cnt]['xR'] =  $_xR;
      }else{
      // Store the 32bit xL2 and xR2
        $code[$code_cnt]['xL2'] =  $_xL;
        $code[$code_cnt]['xR2'] =  $_xR;
        $code_cnt++;
      }
    ...
    // Sum Random Array Numbers
    $rand_sum = array_sum($fish->rand);


    // Start time for encrypting
    $start = microtime(true);
//    echo "Start Encipher<br />";

    foreach($code as $x){
      // Check if sum of array numbers is odd/even
      if($rand_sum%2 == 0){
//        echo "Random Number $rand_sum is Even<br />";
        $fish->_encipher($x['xL'], $x['xR'], $fish->rand);
        $fish->_encipherInverted($x['xL2'], $x['xR2'], $fish->rand);

        $ciphertext .= pack('N2',$x['xL'], $x['xR']); 
        $ciphertext .= pack('N2', $x['xL2'], $x['xR2']); 
      }else{
//        echo "Random Number $rand_sum is Odd<br />";
        $fish->_encipher($x['xL2'], $x['xR2'], $fish->rand);
        $fish->_encipherInverted($x['xL'], $x['xR'], $fish->rand);
        $ciphertext .= pack('N2', $x['xL2'], $x['xR2']); 
        $ciphertext .= pack('N2',$x['xL'], $x['xR']); 
      }
    ...
    // End time for encrypting
    $end = microtime(true);
    unset($fish);
    return array('ciphertext' => $ciphertext, 'start'=>$start, 'end' => $end, 'rand'=> $rand);
  }
   ...
   * @author Matt Harris
   **/
  static function decrypt($ciphertext, $key, $random=array(), $mode=Blowfish128::BLOWFISH_MODE_CBC, $padding=Blowfish128::BLOWFISH_PADDING_RFC, $iv=NULL) {
    if ( $mode == Blowfish128::BLOWFISH_MODE_CBC and empty($iv) ) {
      throw new Exception('CBC Mode requires an IV key');
      return;
    }
    ...
    for ($i=0; $i < $len; $i+=$block) {
      ...
      list(, $_xL, $_xR) = unpack('N2', substr($ciphertext, $i, $block));
      if($cnt%2 == 0){
      // Store the 32bit xL and xR
        $code[$code_cnt]['xL'] =  $_xL;
        $code[$code_cnt]['xR'] =  $_xR;
...
//    echo "Start Encipher<br />";
    foreach($code as $x){
      // Check if sum of array numbers is odd/even
      if($rand_sum%2 == 0){
        $fish->_decipher($x['xL'], $x['xR'], $fish->rand);
        $fish->_decipherInverted($x['xL2'], $x['xR2'], $fish->rand);

        $plaintext .= pack('N2', $x['xL'], $x['xR']);
        $plaintext .= pack('N2', $x['xL2'], $x['xR2']);
      }else{
        $fish->_decipherInverted($x['xL2'], $x['xR2'], $fish->rand);
        $fish->_decipher($x['xL'], $x['xR'], $fish->rand);
    ...
    $end = microtime(true);
    unset($fish);
    return array('plaintext' => $plaintext, 'start'=>$start, 'end' => $end, 'rand'=>$rand);
  }
  ...
  function _pad($plaintext) {
    ...
    switch ($this->padding) {
      case Blowfish128::BLOWFISH_PADDING_NONE:
        return $plaintext;
      case Blowfish128::BLOWFISH_PADDING_RFC:
        $padder = $pad_len;
        break;
      case Blowfish128::BLOWFISH_PADDING_ZERO:
        $padder = 0;
        break;
    }
   ...
   * @access private
   */
  function _encipher(&$xL, &$xR, $rand = array()) {
    $_xL = $xL;
    $_xR = $xR;
    ...
    for ($i=0; $i < $this->N; ++$i) {
      ...
      // Determine if current round was included in rand
      if(in_array($i, $rand) || !$rand){
          $_xR = $this->_F($_xL) ^ $_xR;
      }
  ...
  /**
   * Blowfish inverted enciphering algorithm
   * @access private
   */
  function _encipherInverted(&$xL, &$xR, $rand = array()) {
    ...
    for ($i=0; $i < $this->N; ++$i) {
      ...
      // Determine if current round was not included in rand
      if(!in_array($i, $rand) || !$rand){
          $_xR = $this->_F($_xL) ^ $_xR;
      }
   ...
   * @access private
   */
  function _decipher(&$xL, &$xR, $rand = array()) {
    $_xL = $xL;
    $_xR = $xR;
   ...
   * @access private
   */
  function _decipherInverted(&$xL, &$xR, $rand = array()) {
